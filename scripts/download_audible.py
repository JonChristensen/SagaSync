...
try:
    ...
    print("Missing dependency: audible", file=sys.stderr)
...
def fetch_library(auth: audible.Authenticator):
    with audible.Client(auth) as client:
        library = audible.Library(client, page_size=100)
        for item in library:
            yield item
